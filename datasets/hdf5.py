...
class SliceBuilder:
    ...
    def _build_slices(dataset, patch_shape, stride_shape):
        ...
        Returns:
            list of slices, i.e.
            [(slice, slice, slice, slice), ...] if len(shape) == 2
        ...
        k_y, k_x = patch_shape
        # make sure that the patch shape is within the dataset size
        k_y = min(k_y, i_y)
        k_x = min(k_x, i_x)

        s_y, s_x = stride_shape
        # iterate over z-axis slice by slice
        z_steps = SliceBuilder._gen_indices(i_z, 1, 1)
        for z in z_steps:
...
class HDF5Dataset(Dataset):
    ...
    def __init__(self, file_path, patch_shape, stride_shape, phase, transformer_config,
        ...
        :param patch_shape: the shape of the patch HxW
        :param stride_shape: the shape of the stride HxW
    ...
    @staticmethod
    def _transform_patches(datasets, idx, transformer):
        transformed_patches = []
        for dataset in datasets:
            # get the label data and apply the transformer
            patch = dataset[idx]
            # squeeze Z dimension
    ...
    def _calculate_mean_std(input):
        ...
        """
        return input.mean(keepdims=True)[0], input.std(keepdims=True)[0]
    ...
    def _check_patch_shape(patch_shape):
        assert len(patch_shape) == 2, 'patch_shape must be a 2D tuple'
...
def get_test_loaders(config):
    ...
    for dataset in datasets:
        ...
        yield DataLoader(dataset, batch_size=batch_size, num_workers=num_workers, collate_fn=my_collate)
