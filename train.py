...
from unet2d.losses import get_loss_criterion
from unet2d.metrics import get_evaluation_metric
from unet2d.model import get_model
from unet2d.trainer import UNet2DTrainer
from unet2d.utils import get_logger
from unet2d.utils import get_number_of_learnable_parameters
...
def _create_trainer(config, model, optimizer, lr_scheduler, loss_criterion, eval_criterion, loaders, logger):
    ...
    if resume is not None:
        ...
        return UNet2DTrainer.from_checkpoint(resume, model,
                                             optimizer, lr_scheduler, loss_criterion,
                                             eval_criterion, loaders,
                                             logger=logger)
    elif pre_trained is not None:
        # fine-tune a given pre-trained model
        return UNet2DTrainer.from_pretrained(pre_trained, model, optimizer, lr_scheduler, loss_criterion,
        ...
        return UNet2DTrainer(model, optimizer, lr_scheduler, loss_criterion, eval_criterion,
...
def main():
    # Create main logger
    logger = get_logger('UNet2DTrainer')
